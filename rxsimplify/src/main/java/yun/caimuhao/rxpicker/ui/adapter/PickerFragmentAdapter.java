...
import yun.caimuhao.rxpicker.utils.RxBus;
...
//        checkImage = new ArrayList<>();
        checkImage = config.getOrignSelected();
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType){
        if(CAMERA_TYPE == viewType) {
            return new CameraViewHolder(LayoutInflater.from(parent.getContext())
                    .inflate(yun.picker.simplify.R.layout.item_camera, parent, false));
        }else {
            return new PickerViewHolder(LayoutInflater.from(parent.getContext())
                    .inflate(yun.picker.simplify.R.layout.item_picker, parent, false));
        }
    }
                ...
                if(!RxPickerManager.getInstance().isCrop()) {
                    if(config.isSingle()) {
                        ...
                        if(checkImage.size() == maxValue && !checkImage.contains(imageItem)) {
                            T.show(holder.itemView.getContext(), holder.itemView.getContext()
                                    .getString(yun.picker.simplify.R.string.max_select, config.getMaxValue()));
                            return;
                        }
                        boolean b = checkImage.contains(imageItem) ? checkImage.remove(imageItem) : checkImage
                                .add(imageItem);
    ...
    private class PickerViewHolder extends RecyclerView.ViewHolder {
        ...
        private PickerViewHolder(View itemView){
            super(itemView);
            imageView = (ImageView)itemView.findViewById(yun.picker.simplify.R.id.iv_image);
            cbCheck = (AppCompatCheckBox)itemView.findViewById(yun.picker.simplify.R.id.cb_check);
        }
