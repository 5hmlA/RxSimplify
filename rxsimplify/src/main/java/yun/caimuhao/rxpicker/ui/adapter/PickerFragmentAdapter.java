  ...
  @Override public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    if (CAMERA_TYPE == viewType) {
      return new CameraViewHolder(
          LayoutInflater.from(parent.getContext()).inflate(yun.picker.simplify.R.layout.item_camera, parent, false));
    } else {
      return new PickerViewHolder(
          LayoutInflater.from(parent.getContext()).inflate(yun.picker.simplify.R.layout.item_picker, parent, false));
    }
  }
      ...
      @Override public void onClick(View v) {
        if (config.isSingle()) {
          ...
          if (checkImage.size() == maxValue && !checkImage.contains(imageItem)) {
            T.show(holder.itemView.getContext(),holder.itemView.getContext().getString(yun.picker.simplify.R.string.max_select, config.getMaxValue()));
            return;
          }
