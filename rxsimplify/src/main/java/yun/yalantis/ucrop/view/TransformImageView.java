...
import yun.yalantis.ucrop.callback.BitmapLoadCallback;
...
import yun.yalantis.ucrop.util.FastBitmapDrawable;
...
public class TransformImageView extends ImageView {
     ...
     *         - image Uri
     * @throws Exception
     *         - can throw exception if having problems with decoding Uri or OOM.
     */
    ...
    public float getMatrixScale(@NonNull Matrix matrix){
        return (float)Math.sqrt(Math.pow(getMatrixValue(matrix, Matrix.MSCALE_X), 2)+Math
                .pow(getMatrixValue(matrix, Matrix.MSKEW_Y), 2));
    }
     ...
     *         - valid Matrix object
     * @param valueIndex
     *         - index of needed value. See {@link Matrix#MSCALE_X} and others.
     * @return - matrix value for index
     */
    protected float getMatrixValue(
            @NonNull Matrix matrix, @IntRange(from = 0, to = MATRIX_VALUES_COUNT) int valueIndex){
